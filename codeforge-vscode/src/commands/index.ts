...
import { getEditorContext } from "../context/workspaceContext.js";
import { CodeForgeError } from "../api/errors.js";

async function promptForInstruction(prompt: string): Promise<string | undefined> {
...
export async function explainCommand(client: CodeForgeApiClient): Promise<void> {
  const ctx = getEditorContext(8000);
  if (!ctx) {
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "CodeForge: Explaining code..." },
    async () => {
      try {
        const explanation = await sendToChat(
          client,
          `You are a helpful coding assistant. Explain the following ${language} code clearly and concisely. Focus on what the code does, how it works, and any notable patterns. File: ${filename}`,
          ctx.hasSelection
            ? `Explain this code:\n\`\`\`${language}\n${ctx.text}\n\`\`\``
            : `Explain this file:\n\`\`\`${language}\n${ctx.text}\n\`\`\``
        );
...
export async function generateCommand(client: CodeForgeApiClient): Promise<void> {
  const ctx = getEditorContext(8000);
  ...
  await vscode.window.withProgress(
    ...
    async () => {
      try {
        ...
        const code = await sendToChat(
          client,
          `You are a helpful coding assistant. Generate code based on the user's instruction. Return ONLY the code in a markdown code block with the appropriate language tag. Do not include explanations.`,
...
export async function fixCommand(client: CodeForgeApiClient): Promise<void> {
  const ctx = getEditorContext(8000);
  if (!ctx) {
  ...
  if (editor) {
    ...
    if (diagnostics.length > 0) {
      diagnosticInfo = "\n\nDiagnostics:\n" + diagnostics
        .map((d: vscode.Diagnostic) => `- Line ${d.range.start.line + 1}: ${d.message}`)
        .join("\n");
    }
  }
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "CodeForge: Fixing code..." },
    async () => {
      try {
        const fixed = await sendToChat(
          client,
          `You are a helpful coding assistant. Fix the bugs in the following ${ctx.language} code. Return the corrected code in a markdown code block. File: ${filename}`,
          `Fix this code:\n\`\`\`${ctx.language}\n${ctx.text}\n\`\`\`${diagnosticInfo}`
        );
...
export async function refactorCommand(client: CodeForgeApiClient): Promise<void> {
  const ctx = getEditorContext(8000);
  if (!ctx) {
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "CodeForge: Refactoring code..." },
    async () => {
      try {
        const refactored = await sendToChat(
          client,
          `You are a helpful coding assistant. Refactor the following ${ctx.language} code according to the user's instruction. Return the refactored code in a markdown code block. File: ${filename}`,
...
export async function generateTestsCommand(client: CodeForgeApiClient): Promise<void> {
  const ctx = getEditorContext(8000);
  if (!ctx) {
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "CodeForge: Generating tests..." },
    async () => {
      try {
        const tests = await sendToChat(
          client,
          `You are a helpful coding assistant. Generate comprehensive unit tests for the following ${ctx.language} code. Return the test code in a markdown code block with the appropriate language tag. Use common testing frameworks for the language. File: ${filename}`,
          `Generate tests for:\n\`\`\`${ctx.language}\n${ctx.text}\n\`\`\``
        );
